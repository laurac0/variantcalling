...
workflow BAM_VARIANT_CALLING_HAPLOTYPECALLER {
    ...
    // ch_case_info        // channel: [mandatory] [ val(meta) ]

    main:
    ch_versions = Channel.empty()

    // Run haplotypecaller
    GATK4_HAPLOTYPECALLER(ch_bam_bai, ch_fasta, ch_fai, ch_dict, ch_dbsnp, ch_dbsnp_tbi)
    ...
    GATK4_MERGEVCFS(haplotypecaller_vcf.intervals.map{ meta, vcf -> [ groupKey(meta, meta.num_intervals), vcf ] }.groupTuple(), ch_dict)
    ...
    tbi   // = TABIX_SEN.out.tbi             // channel: [ val(meta), path(tbi) ]
